<template>
    <q-dialog v-model="popupContratoClient" persistent transition-show="scale" transition-hide="scale" full-width>
      <div>
        <q-card>
          <q-card-section>
              <q-table
                  ...
                  <q-tr v-show="props.expand" :props="props">
                    <q-td colspan="100%">
                      <div class="q-pa-md q-gutter-sm">
                        ...
                        <q-btn push color="white" @click="selectionClient(props.row.id)" icon="fact_check" text-color="indigo" label="Agregar al contrato" />
...
export default {
  ...
  computed: {
    ...mapState(['userToken','popupContratoClient'])
  },
  methods: {

    ...mapMutations(['modalCreateCliente','modalUpdateCliente','popupContClient','clientContratSelection']),

    selectionClient(id) {
      let filterCliente = this.data.find( client => client.id === id )
      this.clientContratSelection(filterCliente)
      this.popupContClient(false)
    },
