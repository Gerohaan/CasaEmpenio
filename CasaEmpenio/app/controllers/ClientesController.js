    ...
    async delete(req, res) {
      ...
      let client = await Cliente.findByPk(req.params.id);
        ...
        if(!client) {
            ...
            let clientContrato = await ContratoEmpenio.findAndCountAll(
                { where: { id_cliente: req.params.id } 
            })
            let totolClient = clientContrato.count 
            if(totolClient >= 1)
            {
                res.status(500).json({ msg: "El cliente tiene contratos regitrados" });
            } else {
